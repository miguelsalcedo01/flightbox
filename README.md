...
- **Governance.** Spend caps that *stop the run*: `max_run_cost` on the roster and `max_cost` per agent, enforced at call boundaries — the crossing send is billed and traced, then no further send starts. **Approval phases** that block the run on a human: `ph.approval(...)` parks the chain on a pending row until someone grants it — interactively at the terminal, or from anywhere with `just approve <adw_id> <name>` (`just pending` lists what's waiting; silence past the timeout is a denial, because a gate that defaults open is not a gate). Plus per-agent write boundaries enforced in code with automatic rollback, protected paths no agent may touch, gates as the definition of done, and bounded retry loops. Pre-run cost estimates and spend roll-ups are the active roadmap — see `FLIGHTBOX_PLAN.md`.
...
Early. The core runway (orchestration, trace, replay, cost attribution, write-boundary enforcement) works today. Spend gates, approval workflows, pre-run cost estimates, and the hosted multi-repo dashboard are in progress — roadmap and tiering live in `FLIGHTBOX_PLAN.md`.

## Credits & license

MIT — see [`LICENSE`](LICENSE).

Flightbox is a fork of [Super Simple Software Factory](https://github.com/disler/super-simple-software-factory) by IndyDevDan, whose architecture — code owns the loop, agents own the phases, every run leaves a trace — is the foundation this product builds on. Not affiliated with or endorsed by the original author.
