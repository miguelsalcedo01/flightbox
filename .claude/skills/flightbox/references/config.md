...
The full `flightbox.config.yaml` spec: every field, how defaults merge, and how model / thinking / tools / extensions map onto the coding agent.

It lives at **`adws/adw_flightbox_config/flightbox.config.yaml`** — the default path every `adw_*.py` and the justfile resolve, and where `install.py` / `make_config.py` stamp it. Pass `--config <path>` to any ADW (or set `FLIGHTBOX_CONFIG` for the justfile) to run against a different roster.
...
| Field | Type | Meaning |
|---|---|---|
| `db` | path | SQLite trace db. `tracer.py` writes it directly; the visualizer polls it. Default `adws/adw_data/flightbox.db`. |
...
```yaml
defaults:
  protected_files: [adws/adw_modules/, adws/adw_flightbox_config/, "adws/adw_*.py"]
