...
`/flightbox install` — stamp the entire factory out of the skill and into the current working directory.
...
Run from the **target repo root** — the cwd is where everything lands. If the skill lives in your user scope, the path is `~/.claude/skills/flightbox/scripts/install.py`.
...
|---|---|---|
| `adws/adw_flightbox_config/flightbox.config.yaml` | `templates/flightbox.config.yaml` | yes — the agent roster |
...
| `adws/adw_data/sessions/`, `adws/adw_data/flightbox.db` | created at runtime | no — gitignored |
...
4. **Gitignore** — `install.py` appends `adws/adw_data/sessions/`, `adws/adw_data/flightbox.db*`, and `.env` for you; confirm they landed. All three are runtime or secrets and must never be committed.
...
Green means the whole path works: config validated, session minted, Pi ran, envelope parsed, events landed in `adws/adw_data/flightbox.db`. Verify the trace exists before trusting anything larger:

```bash
sqlite3 adws/adw_data/flightbox.db "select adw_id, status from sessions order by started_at desc limit 1;"
```
