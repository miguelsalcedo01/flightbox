...
1. **What agents, in what order?** Pick from the roster (`adws/adw_flightbox_config/flightbox.config.yaml`). The starter six cover most chains:
...
```bash
uv run .claude/skills/flightbox/scripts/make_adw.py --name review_docs --agents scout,builder
```
...
def main(prompt: str, config: str = "adws/adw_flightbox_config/flightbox.config.yaml", adw_id: str | None = None) -> int:
    ...
    with run.phase(PhaseParams(name="commit", kind="code", owner="git",
        ...
        message = build.commit_message or f"flightbox({run.adw_id}): {build.summary}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="adws/adw_flightbox_config/flightbox.config.yaml")
...
2. Check the trace: `sqlite3 adws/adw_data/flightbox.db "select seq,name,kind,owner,status from phases where adw_id='<id>' order by seq;"`
