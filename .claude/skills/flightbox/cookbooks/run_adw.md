...
```
adws/adw_flightbox_config/flightbox.config.yaml
    planner     fireworks/accounts/fireworks/models/kimi-k3
    builder     google/gemini-3.6-flash (inherited)
adws/adw_flightbox_config/flightbox.frontier.config.yaml
...
| nothing about models at all | the default, `adws/adw_flightbox_config/flightbox.config.yaml` |

`--config` takes the path directly; the justfile recipes read `FLIGHTBOX_CONFIG` instead:

```bash
uv run adws/<chain>.py "<prompt>" --config adws/adw_flightbox_config/flightbox.frontier.config.yaml
FLIGHTBOX_CONFIG=adws/adw_flightbox_config/flightbox.frontier.config.yaml just <recipe> "<prompt>"
```
...
The trace db is `adws/adw_data/flightbox.db`. It is WAL, so reads never block the running writers — poll it as often as you like.

```bash
# where the run stands
sqlite3 adws/adw_data/flightbox.db \
