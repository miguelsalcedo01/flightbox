...
| Module | Owns |
|---|---|
| `data_types.py` | Every Pydantic model: `AgentCall`, `PhaseParams`, `Phase`, `EnvelopeBase` + one output type per agent call, the config models (`AgentConfig`, `FLIGHTBOXConfig`), `EventRecord`, and `PiRequest`/`PiResult` |
...
| `tracer.py` | append JSONL **and** insert every event into `flightbox.db` as it happens |
...
Modules report through `run.console` — never a bare `print()`. Each console method prints a rich line **and** writes it to `flightbox.db` as a `log` event with payload `{message, level}`, both from one `_emit` helper, so the terminal narrative and the swim-lane UI can't drift. New output means a new method on `Console`, not a print at the call site.
