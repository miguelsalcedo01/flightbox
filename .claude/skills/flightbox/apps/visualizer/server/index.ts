/**
 * FLIGHTBOX visualizer server — JSON API over a target repo's flightbox.db, plus the
 ...
 *   bun run server/index.ts --db /path/to/repo/adws/adw_data/flightbox.db
 *   FLIGHTBOX_DB=/path/to/flightbox.db PORT=4600 bun run server/index.ts
 */
import { existsSync, statSync } from "node:fs";
import { join, resolve, sep } from "node:path";
import { FlightboxDb, resolveDbPath } from "./db.ts";
...
let db: FlightboxDb;
try {
  db = new FlightboxDb(dbPath);
} catch (error) {
  console.error(`[flightbox] ${(error as Error).message}`);
  process.exit(1);
}
...
function safely(
  ...
  return async (req) => {
    try {
      ...
      console.error(`[flightbox] ${req.method} ${new URL(req.url).pathname}:`, error);
...
async function serveStatic(req: Request): Promise<Response> {
  ...
  if (!existsSync(DIST_DIR)) {
    return new Response(
      `FLIGHTBOX visualizer API is running on :${PORT}.\n\n` +
...
console.log(`[flightbox] visualizer api  http://localhost:${server.port}`);
console.log(`[flightbox] db              ${db.path}  [journal_mode=${db.journalMode}]`);
console.log(
  existsSync(DIST_DIR)
    ? `[flightbox] serving ui from  ${DIST_DIR}`
    : `[flightbox] no ./dist — use "bun run dev" for the Vite dev server on :4601`,
);
