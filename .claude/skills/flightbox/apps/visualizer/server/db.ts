/**
 * SQLite reader over a target repo's flightbox.db.
 *
...
const DEFAULT_DB_RELATIVE = "adws/adw_data/flightbox.db";
const MAX_LIMIT = 1000;
const DEFAULT_LIMIT = 500;

/**
 * Resolve the db path: --db arg wins, then FLIGHTBOX_DB, then <cwd>/adws/adw_data/flightbox.db.
...
export function resolveDbPath(argv: string[] = Bun.argv): string {
  ...
  const raw =
    ...
    process.env.FLIGHTBOX_DB ??
    DEFAULT_DB_RELATIVE;

  return isAbsolute(raw) ? raw : resolve(process.cwd(), raw);
}

export class FlightboxDb {
   ...
   * `adws/adw_data/flightbox.db`), so deriving it as a sibling of the db file keeps
  ...
  constructor(path: string) {
    if (!existsSync(path)) {
      throw new Error(
        `flightbox.db not found at ${path}\n` +
          `Point the visualizer at a target repo: --db <path> or FLIGHTBOX_DB=<path>, ` +
