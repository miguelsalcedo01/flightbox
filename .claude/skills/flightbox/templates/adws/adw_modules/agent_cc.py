...
same id. FLIGHTBOX's session ids (`flightbox-<adw_id>-<agent>-<hex>`) are not UUIDs, so
...
Model naming convention: flightbox configs write `provider/model-id` and validate()
...
Deltas from pi, on purpose:
  ...
  * `tools:` allowlists are translated (read->Read, bash->Bash, find->Glob, …)
    ...
    FLIGHTBOX's own permissions.py (diff + rollback) stays the enforcement boundary.
...
# ── session mapping (flightbox session id -> Claude session UUID) ─────────────────

def _claude_uuid(session_id: str) -> str:
    """Deterministic UUID for an flightbox session id — stable across processes, so
    a crashed run resumed by adw_id maps back to the same Claude session."""
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"flightbox-claude-code:{session_id}"))
...
def _record_session(request: PiRequest, claude_sid: str) -> None:
    ...
    marker.write_text(json.dumps({"claude_session_id": claude_sid,
                                  "flightbox_session_id": request.session_id,
