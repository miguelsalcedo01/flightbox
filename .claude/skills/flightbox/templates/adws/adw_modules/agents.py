...
from .data_types import (AgentCall, AgentConfig, EnvelopeBase, EventRecord,
                         GateCheck, GateReport, Phase, PiRequest, FLIGHTBOXConfig,
                         UsageBreakdown)
...
def load_config(path: str = "adws/adw_flightbox_config/flightbox.config.yaml") -> FLIGHTBOXConfig:
    ...
    return FLIGHTBOXConfig(**raw)


def resolve(cfg: FLIGHTBOXConfig, name: str) -> AgentConfig:
...
def validate(cfg: FLIGHTBOXConfig, required: list[str]) -> None:
...
def _agent_session_id(run, agent: AgentConfig) -> str:
    ...
    return f"flightbox-{run.adw_id}-{agent.name}-{new_id(4)}"
