---
name: flightbox
description: Flightbox — deploy and operate repeatable agents+code workflows (ADWs) in any codebase. Use when the user says /flightbox install, wants to create/run/update an ADW, manage the agent roster in flightbox.config.yaml, or observe running agent workflows. Keywords - flightbox, software factory, ADW, AI developer workflow, agent pipeline, install factory.
...
1. Read [cookbooks/flightbox_overview.md](cookbooks/flightbox_overview.md) — the system map.
...
- Observe by querying `adws/adw_data/flightbox.db` (WAL — reads never block writers) **when observing is the task**. This is a capability, not a startup step: query it to follow a run you launched or one the engineer asked about, never to volunteer a status report nobody requested.
...
| Request | Cookbook |
|---|---|
| `/flightbox install`, set up the factory in this repo | [cookbooks/install.md](cookbooks/install.md) |
